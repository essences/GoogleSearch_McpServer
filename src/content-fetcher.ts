...
export class ContentFetcher {
  async fetchAndAnalyze(url: string): Promise<PageContent> {
    try {
      ...
      const metadata = {
        ...
        keywords: this.getMetaContent(document, 'keywords')?.split(',').map((k: string) => k.trim()),
      ...
      console.log('Main content tag:', mainContent.length > 0 ? (mainContent.get(0) as any)?.name || 'unknown' : 'none');
      ...
      if (error instanceof Error) {
        if (error.message.includes('Unexpected response format')) {
          throw new McpError(
            ...
            `ページの解析に失敗しました：サーバーからの応答形式が無効です。`,
        ...
        throw new McpError(
          ErrorCode.InternalError,
          `コンテンツの解析中にエラーが発生しました：${error.message}`,
          { originalError: error }
        );
      }
  ...
  private handleAxiosError(error: AxiosErrorLike, url: string): McpError {
    if (error.code === 'ENOTFOUND') {
      return new McpError(
        ErrorCode.InvalidRequest,
        `指定されたURL（${url}）は存在しないか、アクセスできません。`,
        { code: error.code, originalUrl: url }
      );
    }
